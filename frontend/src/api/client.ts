...
export async function request<T>(path: string, init: RequestInit = {}): Promise<T> {
  ...
  try {
    ...
    throw new ApiError(0, "Unable to reach Eventix. Check your connection and try again.");
  }
  ...
  if (!response.ok) {
    ...
    if (response.status === 401) window.dispatchEvent(new Event("eventix:unauthorized"));
