...
export function ShowCard({ show, movie, event, availableSeats, priority = false }: { show: Show; movie?: Movie; event?: Event; availableSeats?: number; priority?: boolean }) {
  ...
  return <article className="show-card">
    ...
    <div className="show-card__body"><div className="eyebrow">{description || (show.showType === "MOVIE" ? "On the big screen" : "An unmissable moment")}</div><h3><Link to={`/shows/${show.id}`}>{show.title}</Link></h3><p className="show-card__line"><CalendarDays size={15} />{dateTime(show.showDateTime)}</p><p className="show-card__line"><MapPin size={15} />{show.venueName}</p>{availableSeats !== undefined && <p className={`show-card__availability ${availableSeats === 0 ? "sold-out" : ""}`}><Users size={14} />{availableSeats === 0 ? "Sold out" : `${availableSeats} tickets left`}</p>}<div className="show-card__bottom"><strong>{money(show.price)} <small>/ ticket</small></strong><Link className="card-arrow" to={`/shows/${show.id}`} aria-label={`Book ${show.title}`}><ArrowUpRight size={18} /></Link></div></div>
  </article>;
}
